...
def proximo_nivel(mundo, invasores, fondo2, ventana, tablero, heroe, clock, estrellas, estrellas2, aster, etapa,
				  tiempo_j):
	...
	def imprime(self, fondo):
		...
		if self.ex == True:
			...
			for i in range(0, 50):  # 35
				pygame.draw.circle(fondo, color[random.randint(0, 1)], (random.randint(x, x1), random.randint(y1, y)),
								   1, 0)
	...
	def actualizar(self, fondo, pause, tiempo_j, mundo):
		...
		if self.activo == False and self.tiempo != None:
			if difere(self.tiempo, tiempo_j) > self.espera:
				self = items(self.img_[0], self.img_[1], self.img_[2], self.img_[3], self.img_[4],
							 nivel.item_t[mundo - 1])
				self.prod()
...
class nave:  # clase del vehiculo espacial
	...
	def lanzar_misil(self):
		...
		if self.misiles == 1:
			...
			self.pos_m = [[self.pos[0] + 17, self.pos[1] - 5], [self.pos[0] + 53, self.pos[1] - 5],
						  [self.pos[0] + 17, self.pos[1] + 35], [self.pos[0] + 53, self.pos[1] + 35]]
	...
	def turb_misil(self, fondo):  #
		if self.ex_misil:
			...
			for i in self.pos_m:
				for e in range(0, 2):
					pygame.draw.circle(fondo, color[random.randint(0, 1)],
									   (random.randint(i[0], i[0] + 3), random.randint(i[1] + 20, i[1] + 23)), 1, 0)
	...
	def imprime_turbina(self, fondo):
		if self.ex_nave:
			...
			for i in range(0, 10):
				pygame.draw.circle(fondo, color[random.randint(0, 1)], (
				random.randint(self.pos[0] + 29, self.pos[0] + 47),
				random.randint(self.pos[1] + 50, self.pos[1] + 100)), 1, 0)
...
class tab:
	...
	def __init__(self, vida, misil, escudo, nivel, imagenes, img_tablero):
		...
		self.combo = [[self.c_vida, (868, 69), vida], [self.c_misil, (868, 166), misil],
					  [self.c_escudo, (871, 261), escudo], [self.c_nivel, (849, 557), nivel]]
...
class grupo_es:
	...
	def __init__(self, incremento=1):
		self.puntos = puntos()
		self.fin = False
		self.color = [(0, 0, 0), (2, 166, 162), (0, 0, 0), (235, 244, 53), (0, 0, 0), (255, 255, 255), (255, 160, 66),
					  (0, 0, 0), (255, 159, 255), (254, 47, 41), (0, 0, 0)]
...
class bombardero:
	...
	def __init__(self, imagenes, tiempo, bm_b):
		...
		self.pos_b = [[self.pos[0] - 10, self.pos[1] + 31], [self.pos[0] + 21, self.pos[1] + 31],
					  [self.pos[0] + 52, self.pos[1] + 31]]
		self.rec_b = None
		self.inc = [-10, 21, 52]
		self.bombas = 3
		self.bombas_ac = []
	...
	def aut_lanzar(self):
		...
		l = len(self.inc)
		if (self.bombas != 0 and a == 1) and (
				(self.direccion == 0 and self.pos[0] >= 40) or (self.direccion == 1 and self.pos[0] <= 680)):
...
class batallon:
	def __init__(self, cantidad, tiempo_prod):
		...
		self.rec1 = self.recta(self.x1_rec1, self.x2_rec1, 0, 130,
							   0)  # puntos primera recta(14 puntos) #cir 13 puntos 13+14x2=41 puntos trayectoria total
		self.cir = [[self.x2_rec1 - 46, 130 + 19], [self.x2_rec1 - 65, 130 + 65], [self.x2_rec1 - 46, 130 + 111],
					[self.x2_rec1, 130 + 130], [self.x2_rec1 + 46, 130 + 111], [self.x2_rec1 + 65, 130 + 65],
					[self.x2_rec1 + 46, 130 + 19], [self.x2_rec1, 130], [self.x2_rec1 - 46, 130 + 19],
					[self.x2_rec1 - 65, 130 + 65], [self.x2_rec1 - 46, 130 + 111], [self.x2_rec1, 130 + 130]]
